...
- DB2 场景化兜底输出，覆盖索引变更、新增字段、数据修复、REORG/RUNSTATS、锁等待应急、HADR 受控切换、表空间扩容和权限收敛
- 交付完成度、待确认项总览和复核进度提示
...
应用即使没有配置模型 Key 也能完整生成交付包。未设置 `LLM_API_KEY` 或模型调用失败时，系统会记录失败原因，并按案例类型使用内置合成 DBA 输出兜底，避免试运行、评审或现场讲解受网络、额度或模型服务影响。当前兜底内容已区分 DB2 索引变更、新增字段、数据修复、REORG/RUNSTATS、锁等待应急、HADR 受控切换、表空间扩容和权限收敛等典型场景。
