...
- 工单回写审计脱敏：Webhook 发送仍使用真实配置，但日志/API 会遮蔽 URL 查询密钥、Basic Auth 密码和外部响应体中的敏感字段
...
- LLM 和工单回写审计会在落库前脱敏常见密码、Token、API Key、连接串口令、Webhook URL 查询密钥和外部响应体敏感字段，兼顾可追溯性和公开演示安全边界。
