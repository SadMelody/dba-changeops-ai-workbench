...
- 变更沟通摘要
- AI 调用审计
- LLM 审计脱敏：落库前遮蔽常见密码、Token、API Key、Access Key、Signature、Authorization/Cookie/session 字段、敏感键名和连接串口令
- 工单回写审计脱敏：Webhook 发送仍使用真实配置，但日志/API 会遮蔽 URL 查询串和 fragment 密钥、Basic Auth 密码、Authorization/Cookie/session 字段、外部响应体敏感字段，以及外部工单 labels/metadata 中的敏感内容
...
这个项目的核心不是“调用一次模型”，而是把数据库变更从外部输入推进到可交付证据：

| 阶段 | 系统动作 | 可展示证据 |
| --- | --- | --- |
| 输入 | 手工创建案例，或通过外部工单导入 API 把 ITSM/Jira 类 payload 归一化为变更案例 | 案例详情、业务背景、SQL、表结构、约束条件 |
| 生成 | 调用 OpenAI-compatible 模型；无 Key、超时或失败时进入 DB2 场景化兜底 | 6 类交付物、LLM 调用审计、兜底状态 |
| 复核 | DBA 编辑交付物，保存版本快照，查看最近内容差异 | 版本记录、diff 摘要、交付完成度 |
| 确认 | 单份确认或整包确认，确保所有交付物进入可交付状态 | 6/6 确认进度、确认版本号 |
| 签收 | 审批人对一次分析运行生成的整包材料签收；签收后再编辑会回到待签收 | 签收人、签收时间、签收说明 |
| 交付 | 导出 Markdown/PDF，或生成标准工单回写 payload 并通过通用 Webhook 发送 | 导出文件、回写 payload、回写日志、失败重试记录 |

面试讲解时可以把这条链路概括成：外部输入不是直接交给模型拍板，而是先被归一化为案例；AI 只负责生成可复核初稿；DBA 的编辑、确认和签收决定最终口径；导出和回写负责把结果带回评审、归档或外部工单系统。
...
- LLM 和工单回写审计会在落库前脱敏常见密码、Token、API Key、Access Key、Signature、Authorization/Cookie/session 字段、敏感键名、连接串口令、Webhook URL 查询串/fragment 密钥、外部工单 labels/metadata 和外部响应体敏感字段，兼顾可追溯性和公开演示安全边界。
