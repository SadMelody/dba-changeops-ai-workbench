...
- **工单集成闭环**：支持外部工单导入、标准回写 payload、可配置 ITSM Webhook、回写 attempt 日志和失败重试。
...
- 设计案例、分析运行、交付物、版本记录、LLM 调用日志和工单回写日志等核心数据模型，支持 AI 生成、人工编辑、版本差异、单项确认、整包确认、签收、Webhook 回写和 Markdown/PDF 导出。
