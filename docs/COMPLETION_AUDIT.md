...
| Webhook 审计脱敏 | `sanitize_webhook_url`、`sanitize_audit_payload`、`work_order_writeback_logs` | URL 查询串/fragment 密钥、Basic Auth 密码、Authorization/Cookie/session 字段和外部响应体脱敏测试 |
...
| 工单回写审计脱敏 | `sanitize_webhook_url`、`sanitize_audit_payload` | Webhook URL 查询串/fragment 密钥、Basic Auth 密码、Authorization/Cookie/session 字段和外部响应体脱敏测试 |
