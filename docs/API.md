...
审计边界：实际发送会使用原始 `ITSM_WEBHOOK_URL` 和可选 Bearer Token；API 响应和 `work_order_writeback_logs` 中保存的 URL 会遮蔽查询串中的 token/key/signature 类字段和 Basic Auth 密码，外部响应体、异常消息和审计 payload 中的 Authorization、Basic Auth、Cookie、credential、session、密码、API Key 和 Token 字段也会按审计规则脱敏后保存。
